<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
/** @var array $arParams */
/** @var array $arResult */
/** @global CMain $APPLICATION */
/** @global CUser $USER */
/** @global CDatabase $DB */
/** @var CBitrixComponentTemplate $this */
/** @var string $templateName */
/** @var string $templateFile */
/** @var string $templateFolder */
/** @var string $componentPath */
/** @var CBitrixComponent $component */
$this->setFrameMode(true);

$img = '';
if(isset($arResult["DETAIL_PICTURE"])){
    $img = $arResult["DETAIL_PICTURE"]['SRC'];
}else{
    $img = SITE_TEMPLATE_PATH . '/img/rew/no_photo.jpg';
}
?>

<div class="review-block">
    <div class="review-text">
        <div class="review-text-cont">
            <?echo $arResult["DETAIL_TEXT"];?>
        </div>
        <div class="review-autor">
            <?=$arResult['NAME']?>,
            <?=$arResult['ACTIVE_FROM']?>,
            <?=$arResult['PROPERTIES']['POSITION']['VALUE']?>,
            <?=$arResult['PROPERTIES']['COMPANY']['VALUE']?>
        </div>
    </div>
    <div style="clear: both;" class="review-img-wrap"><img src="<?=$img?>" alt="img"></div>
</div>
<div class="exam-review-doc">
    <? if(is_array($arResult['PROPERTIES']['DOCUMENTS']['VALUE']) &&
        count($arResult['PROPERTIES']['DOCUMENTS']['VALUE']) > 0) { ?>
        <p>Документы:</p>
        <?foreach ($arResult['PROPERTIES']['DOCUMENTS']['VALUE'] as $documentID){
            $file = CFile::getFileArray($documentID);?>
            <div class="exam-review-item-doc">
                <img class="rew-doc-ico" src="<?=SITE_TEMPLATE_PATH?>/img/icons/pdf_ico_40.png">
                <a href="<?=$file['SRC']?>"><?=$file['ORIGINAL_NAME']?></a>
            </div>

        <? } ?>
    <?}?>
</div>
<hr>
<a href="/rew/" class="review-block_back_link"><?=GetMessage('BACK_TO_LIST')?></a>
